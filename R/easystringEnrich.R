...
easystringEnrich=function(
 ...
 #背景蛋白 如果有就进行Id整合 没有就默认全部的背景
  ...
  result=data.table::as.data.table(proteincounts.merged)[, p_value := 1 - stats::phyper(
    number_of_genes - 1,                #差异蛋白数 - 1
    diff.proteincounts,                                #差异蛋白总数
    background.proteincounts - diff.proteincounts,    #背景蛋白总数减去差异蛋白总数
    number_of_genes_in_background  ,                    #通路上背景蛋白总数
    lower.tail=FALSE
  )]
  result=data.table::as.data.table(result)[, FDR := p.adjust(p_value, method = "BH")]
  result=data.table::as.data.table(result)[, Enrich_factor := round(number_of_genes/number_of_genes_in_background,3)]
